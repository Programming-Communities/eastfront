...
export default async function NotFound() {
  ...
  const requestedPath = pathname || '/unknown-path';

  // Navigation suggestions
  const suggestions = [
    { name: 'Home', path: `/${currentLocale}`, icon: Home },
    { name: 'About Us', path: `/${currentLocale}/about`, icon: HelpCircle },
    { name: 'Digital Library', path: `/${currentLocale}/books`, icon: Search },
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-950 via-black to-gray-900 text-white">
      <div className="text-center p-8 max-w-2xl">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-8">
          {suggestions.map((suggestion) => (
            <Link
              ...
              className="group flex items-center justify-center gap-2 px-4 py-3 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-red-500/30 rounded-xl transition-all duration-300 text-sm font-medium text-gray-300 hover:text-white"
            >
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <Link 
            href={`/${currentLocale}`}
            className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-gradient-to-r from-red-600 to-orange-600 text-white rounded-xl hover:from-red-700 hover:to-orange-700 transition-all font-medium shadow-lg shadow-red-500/20"
          >
